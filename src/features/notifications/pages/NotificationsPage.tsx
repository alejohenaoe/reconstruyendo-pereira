...
export function NotificationsPage() {
  const { notifications, unread, loading, loadingMore, hasMore, error, reload, loadMore, markRead, markAllRead, remove } =
    useNotifications()
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-closed-500 mt-1 text-sm">
            {unread > 0 ? `Tienes ${unread} ${unread === 1 ? 'notificación' : 'notificaciones'} por leer.` : 'Estás al día.'}
          </p>
        </div>
        {unread > 0 ? (
          <button type="button" onClick={() => void markAllRead()} className={buttonStyles({ variant: 'subtle', size: 'sm' })}>
