...
function mapCommunityError(
  ...
  if (message.includes('no acepta nuevas ofertas')) {
    return 'Este pedido de ayuda ya fue solucionado y ya no acepta nuevas ofertas.'
  }
  if (message.includes('No puedes ofrecer ayuda en tu propia')) {
    return 'No puedes ofrecer ayuda en tu propio pedido de ayuda.'
  }
  if (message.includes('no existe') || message.includes('no está disponible')) {
    return 'Este pedido de ayuda ya no está disponible.'
  }
  if (code === '23505') {
    return 'Ya te ofreciste a ayudar en este pedido de ayuda.'
  }
  if (message.includes('Solo el autor puede cambiar')) {
    return 'Solo el autor puede cambiar el estado del pedido de ayuda.'
  }
  if (message.includes('Transición de estado no permitida')) {
    return 'No puedes pasar el pedido de ayuda a ese estado.'
  }
  if (message.includes('no puede reabrirse')) {
    return 'Un pedido de ayuda solucionado o cerrado no puede reabrirse.'
  }
  return fallback
}
...
export async function getCapabilities(): Promise<HelpResult<Capability[]>> {
  const { data, error } = await supabase
    ...
    .order('id')
  if (error)
    return { ok: false, data: null, error: 'No pudimos cargar las capacidades.', code: 'unknown' }
...
export async function getOfferers(needId: string): Promise<HelpResult<Offerer[]>> {
  const { data, error } = await supabase
    ...
    .select(
      'offer_id, user_id, display_name, offer_status, message, capability_id, capability_label, offered_at',
    )
    .eq('need_id', needId)
    .order('offered_at', { ascending: true })
    .limit(100)
  if (error)
    return { ok: false, data: null, error: 'No pudimos cargar las ofertas.', code: 'unknown' }
...
export async function createHelpOffer(
  ...
  if (error) {
    return {
      ok: false,
      data: null,
      error: mapCommunityError(
        error,
        'No pudimos registrar tu oferta de ayuda. Inténtalo de nuevo.',
      ),
      code: 'unknown',
    }
  }
...
export async function getNeedContact(needId: string): Promise<HelpResult<NeedContact | null>> {
  ...
  if (error)
    return { ok: false, data: null, error: 'No pudimos obtener el contacto.', code: 'unknown' }
  return {
    ok: true,
    data: (data as unknown as NeedContact | null) ?? null,
    error: null,
    code: null,
  }
}
...
export async function addComment(
  ...
  if (error) {
    return {
      ok: false,
      data: null,
      error: mapCommunityError(error, 'No pudimos publicar tu mensaje.'),
      code: 'unknown',
    }
  }
  return { ok: true, data: null, error: null, code: null }
